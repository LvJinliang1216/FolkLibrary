...
namespace FolkLibrary.Util.MvcPager
{
    ...
    public static class PageLinqExtensions
    {
        ...
        public static PagedList<T> ToPagedList<T>(this IQueryable<T> allItems, int pageIndex, int pageSize, int total)
        {
            if (pageIndex < 1)
                pageIndex = 1;
            var itemIndex = (pageIndex - 1) * pageSize;
        ...
        public static PagedList<T> ToPagedList<T>(this IEnumerable<T> allItems, int pageIndex, int pageSize, int total)
        {
            return allItems.AsQueryable().ToPagedList(pageIndex, pageSize, total);
        }
    }
}
